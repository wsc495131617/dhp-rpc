...
public class Crc16 {
	private final static char[] crc_tb = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5,
...
public static short caluCRC(ByteBuffer bytes){
	bytes.position(0);
	char crc;
	byte da;
	crc = 0x0;
...
public static short caluCRCShort(byte group,short cmd,ByteBuffer bytes){
	char crc;
	byte da;
	crc = 0x0;
	int num = 0;
...
public static short caluCRC(byte group,short cmd,ByteBuffer bytes){
	char crc;
	byte da;
	crc = 0x0;
	int num = 0;
...
public static void encode(byte[] bytes,short crc,int offset){
	ByteBuffer buf = ByteBuffer.wrap(bytes);
	encode(buf, crc,offset);
}

public static void encode(ByteBuffer bytes,short crc,int offset){
	byte b;
...
public static void encode(ByteBuffer bytes,short crc){
	byte b;
	bytes.position(0);
	while(bytes.hasRemaining()){
		b = bytes.get();
...
public static void encode(ByteBuffer bytes,char crc){
	decode(bytes,crc);
}

public static void encode(byte[] bytes,short crc){
...
public static ByteBuffer decode2(ByteBuffer bytes,char crc){
	ByteBuffer newbytes = ByteBuffer.allocate(bytes.limit());
	byte b;
	bytes.position(0);
	while(bytes.hasRemaining()){
		b = bytes.get();
...
public static void decode(ByteBuffer bytes,short crc){
	byte b;
	bytes.position(0);
	while(bytes.hasRemaining()){
		b = bytes.get();
...
public static void decode(ByteBuffer bytes,char crc){
	byte b;
	bytes.position(0);
	while(bytes.hasRemaining()){
		b = bytes.get();
